...
export function ModulePage({ slug }: { slug: DashboardModuleSlug }) {
  const moduleInfo = moduleCatalog[slug];

  return (
    <section className="space-y-6">
      <div className="rounded-panel border border-slate-200 bg-white/90 p-6 shadow-panel">
        <p className="text-sm font-semibold uppercase tracking-[0.26em] text-teal-700">{moduleInfo.eyebrow}</p>
        <div className="mt-3 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div className="max-w-3xl">
            <h2 className="text-3xl font-semibold text-brand-ink">{moduleInfo.title}</h2>
            <p className="mt-3 text-sm leading-7 text-slate-600">{moduleInfo.description}</p>
          </div>
          <Link
            href="/dashboard"
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <article className="rounded-panel border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <ul className="mt-4 space-y-3 text-sm text-slate-600">
            {moduleInfo.highlights.map((item) => (
